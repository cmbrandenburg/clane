...
int main() {
	...
	while (true) {
		...
		check_true(net::status::ok == send_result.stat || net::status::would_block == send_result.stat);
		if (send_result.size < strlen(m)) {

			// send fin:
			send_result = cli_sock.send(nullptr, 0, net::op_fin);
			check_eq(net::status::ok, send_result.stat);
			check_eq(static_cast<size_t>(0), send_result.size);
			break;
		}
		check_eq(net::status::ok, send_result.stat);
		check_eq(strlen(m), send_result.size);
	}
	...
	// server receive:
	{
		...
		while (true) {
			...
			for (size_t i = 0; i < recv_result.size; ++i) {
				check_eq(buf[i], static_cast<char>('A' + (cnt % 26)));
				++cnt;
			}
		}
	}
