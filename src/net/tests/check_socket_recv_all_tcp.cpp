...
int main() {
	...
	// server receive:
	// Try to receive more data than are available. The operation should
	// complete with success.
	char buf[10];
	auto recv_result = serv_sock.recv(buf, sizeof(buf), net::op_all);
	check(net::status::ok == recv_result.stat);
	check(static_cast<size_t>(5) == recv_result.size);
	check(0 == memcmp(buf, "HELLO", 5));

	// server receive:
	recv_result = serv_sock.recv(buf, sizeof(buf), net::op_all);
	check(net::status::ok == recv_result.stat);
