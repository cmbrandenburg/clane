...
namespace clane {
	namespace http {
		...
		parser::parser(): stat(state::ready), len_limit(0), cur_len(0), what_(nullptr) {
		}
		...
		void parser::set_error(status_code stat_code, char const *what) {
			stat = state::error;
				...
				switch (cur_phase) {
					case phase::request_line:
						if (!req_line_parser.parse(cur, end - cur)) {
							if (!req_line_parser)
								set_error(req_line_parser.stat_code, req_line_parser.what());
							return false;
						}
					...
					case phase::headers:
						if (!hdrs_parser.parse(cur, end - cur)) {
							if (!hdrs_parser)
								set_error(hdrs_parser.stat_code, hdrs_parser.what());
							return false;
						}
