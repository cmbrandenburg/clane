...
// This URI implementation is based on RFC 3986. BNF syntax rules noted in the
// code comments here derive from that document.
...
namespace clane {
	namespace uri {
		...
		static bool has_prefix(char const *beg, char const *end, char const *key) {
			assert(beg <= end);
		...
		static bool is_hexdig_char(char c) {
		...
		static bool is_sub_delim_char(char c) {
			return
				c == '!' ||
				c == '$' ||
				c == '&' ||
		...
		static bool is_scheme_nonfirst_char(char c) {
		...
		static bool is_ipvfut_address_char(char c) {
			return is_unreserved_char(c) || is_sub_delim_char(c) || c == ':';
		}
		...
		static bool is_pchar_char(char c) {
		...
		static size_t scan_h16(char const *beg, char const *end) {
			// h16 = 1*4HEXDIG
			char const *cur = beg;
		...
		bool is_ipvfut_address(char const *beg, char const *end) {
			...
			if (cur == end || *cur != 'v')
				return false;
			++cur;
			char const *delim = std::find_if_not(cur, end, is_hexdig_char);
			if (cur == delim)
				return false;
			cur = delim;
			if (cur == end || *cur != '.')
